...
为行程中的每个地点（活动）增加结构化元数据：特点描述（highlights）、关键词标签（tags）、推荐语（recommendation），由 LLM 在生成行程时一并输出，前端在预览和详情页展示。
...
- 2026-07-03 — initial spec (from add-place-metadata change)
