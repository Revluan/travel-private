...
系统 SHALL 在首页上半部分展示一个飞机舷窗视角的沉浸式动画，窗内景色随用户滚动而变化。

#### Scenario: 用户进入首页看到舷窗
- **WHEN** 用户访问 `/`
- **THEN** 页面中央显示飞机舷窗效果（圆角矩形窗框 + 窗内实景照片）
- **AND** 第一帧显示云海之上的景色

#### Scenario: 用户滚动切换舷窗景色
- **WHEN** 用户在舷窗区域内向下滚动
- **THEN** 窗内照片按顺序切换：云海 → 山脉 → 海岸 → 城市
...
#### Scenario: 未登录用户在舷窗区域看到 CTA
- **WHEN** 未登录用户查看首页
- **THEN** 舷窗动画中展示「规划你的下一次完美旅行」文案
